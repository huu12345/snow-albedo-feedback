...
# This is the land mask file
 ...
 set outdir_root =/home/hujl/SAF_time/NH45_dTNH45/paper3_reply/reviewer3_amip/data_snow_albedo/decadal/result_cmip6 || exit 1
 # Set work directory
 set work =/home/hujl/SAF_time/NH45_dTNH45/paper3_reply/reviewer3_amip/data_snow_albedo/decadal/work_cmip6 || exit 1
 ...
 # Set the snow cover threshold - we will calculate SAF all cells with more than 0.25 snc (previously used 0.1)
  ...
  foreach var (albedo tas snc)
   ...
   if ($#flag != 0 ) set gridstr = "-selgrid,2"
     ...
     cdo ymonmean -selname,$var -setvrange,0.0,100.1 -setmissval,-999.0 -seldate,$dates_pr1 -sellonlatbox,${regstr} $gridstr $infile tmp.snc.pr1.nc || exit 1
     cdo ymonmean -selname,$var -setvrange,0.0,100.1 -setmissval,-999.0 -seldate,$dates_pr2 -sellonlatbox,${regstr} $gridstr $infile tmp.snc.pr2.nc || exit 1
   ...
   if ($var == snc) then
    ...
    cdo divc,100 tmp.snc.pr1.nc tmp.nc && \mv tmp.nc tmp.snc.pr1.nc || exit 1
    cdo divc,100 tmp.snc.pr2.nc tmp.nc && \mv tmp.nc tmp.snc.pr2.nc || exit 1
   endif
  end  # get var loop
  ...
  cdo divc,100 -selname,snc -setvrange,0.0,100.1 -selmon,3,4,5,6 -seldate,$dates_pr1 -sellonlatbox,${regstr} -setmissval,-999.0 $snc_file tmp.snc.all.pr1.nc || exit 1
  ...
  cdo divc,100 -selname,snc -setvrange,0.0,100.1 -selmon,3,4,5,6 -seldate,$dates_pr2 -sellonlatbox,${regstr} -setmissval,-999.0 $snc_file tmp.snc.all.pr2.nc || exit 1
