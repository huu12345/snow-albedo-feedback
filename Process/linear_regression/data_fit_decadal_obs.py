...
# **************读取数据**************
# 输入路径
inpath_model_DS = "/home/hujl/SAF_time/NH45_dTNH45/paper4_reply_noGLASS/reviewer3_amip/data_snow_albedo/decadal/result_obs_all/"
...
# 初始化数组
delta_tas = np.full((2,3,3, 64, n_lat, n_lon), np.nan, dtype=np.float64)
delta_alpsnow = np.full((2,3,3, 64, n_lat, n_lon), np.nan, dtype=np.float64)
delta_prsn = np.full((2,3,3, 64, n_lat, n_lon), np.nan, dtype=np.float64)
snc1 = np.full((2,3,3, 64, n_lat, n_lon), np.nan, dtype=np.float64)
snc2 = np.full((2,3,3, 64, n_lat, n_lon), np.nan, dtype=np.float64)
...
for m in range(2):  #tas
  for n in range(3): #snc
    for k in range(3): #alb
        # 构建文件路径
        file1 = os.path.join(inpath_model_DS, f"delta_tas.obs_{nm_tas[m]}_{nm_snc[n]}_{nm_alb[k]}.EASE_grid.nc")
        file2 = os.path.join(inpath_model_DS, f"delta_alpha_snow.obs_{nm_tas[m]}_{nm_snc[n]}_{nm_alb[k]}.EASE_grid.nc")
        file3 = os.path.join(inpath_model_DS, f"delta_prsn.obs_{nm_tas[m]}_{nm_snc[n]}_{nm_alb[k]}.EASE_grid.nc")
        file4 = os.path.join(inpath_model_DS, f"snc.all.pr1.obs_{nm_tas[m]}_{nm_snc[n]}_{nm_alb[k]}.EASE_grid.nc")
        file5 = os.path.join(inpath_model_DS, f"snc.all.pr2.obs_{nm_tas[m]}_{nm_snc[n]}_{nm_alb[k]}.EASE_grid.nc")

        try:
            if os.path.exists(file1):
                ds1 = xr.open_dataset(file1, decode_times=False)
                # 直接使用数值索引，不依赖时间坐标
                delta_tas[m,n,k, :, :, :] = ds1['tas'].values[:, :, :]
                ds1.close()

            if os.path.exists(file2):
                ds2 = xr.open_dataset(file2, decode_times=False)
                delta_alpsnow[m,n,k, :, :, :] = ds2['alpha_snow'].values[:, :, :]
                ds2.close()

            if os.path.exists(file3):
                ds3 = xr.open_dataset(file3, decode_times=False)
                delta_prsn[m,n,k, :, :, :] = ds3['prsn'].values[:, :, :]
                ds3.close()

            if os.path.exists(file4):
                ds4 = xr.open_dataset(file4, decode_times=False)
                snc1[m,n,k, :, :, :] = ds4['snc'].values[:, :, :]
                ds4.close()

            if os.path.exists(file5):
                ds5 = xr.open_dataset(file5, decode_times=False)
                snc2[m,n,k, :, :, :] = ds5['snc'].values[:, :, :]
                ds5.close()

        except Exception as e:
            print(f"读取观测 {nm_tas[m]}_{nm_snc[n]}_{nm_alb[k]} 时出错: {e}")

# 数据转换
snc1 = snc1*100.0  # snc转换为百分比
snc2 = snc2*100.0  # snc转换为百分比
# delta_prsn = delta_prsn * 24 * 60 * 60 # delta_prsn从mm/s转换为mm/day

# 打印数据范围
# print("\n数据范围:")
print(f"snc1: min={np.nanmin(snc1):.2f}, max={np.nanmax(snc1):.2f}")
print(f"snc2: min={np.nanmin(snc2):.2f}, max={np.nanmax(snc2):.2f}")
...
mask = (snc1 > value) & (snc2 > value)

# 应用掩码
delta_tas_mk1 = np.where(mask, delta_tas, np.nan)
delta_alpsnow_mk1 = np.where(mask, delta_alpsnow, np.nan)
delta_prsn_mk1 = np.where(mask, delta_prsn, np.nan)

# print(f"delta_tas_mk: min={np.nanmin(delta_tas_mk1):.2f}, max={np.nanmax(delta_tas_mk1):.2f}")
...
# 观测均值
delta_tas_mk = safe_nanmean(delta_tas_mk1, axis=(0, 1))  #3, 33, n_lat, n_lon
...
for i in range(0,3):
   ...
   for lat in range(0,n_lat):
      for lon in range(0,n_lon):
          ...
          x = delta_tas_mk[i,:,lat,lon]  #z=ax+by+c
          y = delta_prsn_mk[i,:,lat,lon]  # 转化为一维
          z = delta_alpsnow_mk[i,:,lat,lon]
         ...
         # print(n_valid)
          ...
          if n_valid >= 15:  # 一共16*4=64个样本，至少需要50个样本
...
#****************save output**********************
# out1 = xr.DataArray(exp_var, coords=[np.arange(0,1),np.arange(0,3,1)],dims=['mon'])
...
out8 = xr.DataArray(delta_tas_mk, coords=[np.arange(0,3,1),np.arange(0,64,1),np.arange(0,n_lat,1),np.arange(0,n_lon,1)],dims=['alb','year','lat','lon'])
out9 = xr.DataArray(delta_alpsnow_mk, coords=[np.arange(0,3,1),np.arange(0,64,1),np.arange(0,n_lat,1),np.arange(0,n_lon,1)],dims=['alb','year','lat','lon'])
out10 = xr.DataArray(delta_prsn_mk, coords=[np.arange(0,3,1),np.arange(0,64,1),np.arange(0,n_lat,1),np.arange(0,n_lon,1)],dims=['alb','year','lat','lon'])
...
out.to_netcdf(outpath+'data_fit_decadal_obs.nc')
