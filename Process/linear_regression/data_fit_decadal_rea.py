...
# **************模型列表**************
model6 = np.array(["ERA5-Land", "ERA5snow", "JRA3Q","JRA55", "MERRA2"])
...
# 输入路径
inpath_model_DS = "/home/hujl/SAF_time/NH45_dTNH45/paper4_reply_noGLASS/reviewer3_amip/data_snow_albedo/decadal/"
...
# 初始化数组
delta_tas = np.full((n_model6, 64, n_lat, n_lon), np.nan, dtype=np.float64)
delta_alpsnow = np.full((n_model6, 64, n_lat, n_lon), np.nan, dtype=np.float64)
delta_prsn = np.full((n_model6, 64, n_lat, n_lon), np.nan, dtype=np.float64)
snc1 = np.full((n_model6, 64, n_lat, n_lon), np.nan, dtype=np.float64)
snc2 = np.full((n_model6, 64, n_lat, n_lon), np.nan, dtype=np.float64)

print("\n开始读取数据...")
for i in range(n_model6):
        m6 = model6[i]
        data_folder = f"result_{m6}"  # result_ERA5-Land, result_ERA5-snow, etc.

        # 构建文件路径
        file1 = os.path.join(inpath_model_DS, data_folder, f"delta_tas.{m6}2.EASE_grid.nc")
        file2 = os.path.join(inpath_model_DS, data_folder, f"delta_alpha_snow.{m6}2.EASE_grid.nc")
        file3 = os.path.join(inpath_model_DS, data_folder, f"delta_prsn.{m6}2.EASE_grid.nc")
        file4 = os.path.join(inpath_model_DS, data_folder, f"snc.all.pr1.{m6}2.EASE_grid.nc")
        file5 = os.path.join(inpath_model_DS, data_folder, f"snc.all.pr2.{m6}2.EASE_grid.nc")

        try:
            if os.path.exists(file1):
                ds1 = xr.open_dataset(file1, decode_times=False)
                # 直接使用数值索引，不依赖时间坐标
                delta_tas[i, :, :, :] = ds1['tas'].values[:, :, :]
                ds1.close()

            if os.path.exists(file2):
                ds2 = xr.open_dataset(file2, decode_times=False)
                delta_alpsnow[i, :, :, :] = ds2['alpha_snow'].values[:, :, :]
                ds2.close()

            if os.path.exists(file3):
                ds3 = xr.open_dataset(file3, decode_times=False)
                delta_prsn[i, :, :, :] = ds3['prsn'].values[:, :, :]
                ds3.close()

            if os.path.exists(file4):
                ds4 = xr.open_dataset(file4, decode_times=False)
                snc1[i, :, :, :] = ds4['snc'].values[:, :, :]
                ds4.close()

            if os.path.exists(file5):
                ds5 = xr.open_dataset(file5, decode_times=False)
                snc2[i, :, :, :] = ds5['snc'].values[:, :, :]
                ds5.close()

        except Exception as e:
            print(f"读取模型 {m6} 时出错: {e}")

# 数据转换
snc1 = snc1*100.0  # snc转换为百分比
snc2 = snc2*100.0  # snc转换为百分比
delta_prsn = delta_prsn  # delta_prsn单位为mm/mon，和观测的单位保持一致

# 打印数据范围
print("\n数据范围:")
print(f"snc: min={np.nanmin(snc1):.2f}, max={np.nanmax(snc1):.2f}")
...
# 应用掩码
delta_tas_mk = np.where(mask, delta_tas, np.nan)
delta_alpsnow_mk = np.where(mask, delta_alpsnow, np.nan)
delta_prsn_mk = np.where(mask, delta_prsn, np.nan)
...
out8 = xr.DataArray(delta_tas_mk, coords=[np.arange(0,n_model6,1),np.arange(0,64,1),np.arange(0,n_lat,1),np.arange(0,n_lon,1)],dims=['model','year','lat','lon'])
out9 = xr.DataArray(delta_alpsnow_mk, coords=[np.arange(0,n_model6,1),np.arange(0,64,1),np.arange(0,n_lat,1),np.arange(0,n_lon,1)],dims=['model','year','lat','lon'])
out10 = xr.DataArray(delta_prsn_mk, coords=[np.arange(0,n_model6,1),np.arange(0,64,1),np.arange(0,n_lat,1),np.arange(0,n_lon,1)],dims=['model','year','lat','lon'])
...
out.to_netcdf(outpath+'data_fit_decadal_rea.nc')
