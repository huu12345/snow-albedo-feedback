...
# 输入路径
inpath_model_SS = "/home/hujl/SAF_time/NH45_dTNH45/paper4_reply_noGLASS/reviewer3_amip/data_snow_albedo/seasonal/"
...
for i in range(n_model6):
    for j in range(3):  # 3个季节
        ...
        # 构建文件路径
        file1 = os.path.join(inpath_model_SS, data_folder, M[j], f"delta_tas.{m6}2.EASE_grid.nc")
        file2 = os.path.join(inpath_model_SS, data_folder, M[j], f"delta_alpha_snow.{m6}2.EASE_grid.nc")
        file3 = os.path.join(inpath_model_SS, data_folder, M[j], f"delta_prsn.{m6}2.EASE_grid.nc")
        file4 = os.path.join(inpath_model_SS, data_folder, M[j], f"snc.all.{m6}2.EASE_grid.nc")
        print(file1)
        try:
...
delta_prsn = delta_prsn # delta_prsn单位mm/mon，和观测的单位保持一致
...
out.to_netcdf(outpath+'data_fit_seasonal_rea.nc')
