...
##################################
#
# Snow Albedo Feedback (SAF) on the seasonal timescale
...
set lmask_file = /home/hujl/data_script/mask_greenland_MODIS/MCD12C1/mask_greenland_MCD12C1_1.0x1.0.nc || exit 1
 ...
 set outdir_root = /home/hujl/SAF_time/NH45_dTNH45/review/amip/data/seasonal/result/result_obs_all || exit 1
 # Set work directory
 set work = /home/hujl/SAF_time/NH45_dTNH45/review/amip/data/seasonal/result/work_obs_all || exit 1
...
# Set the snow cover threshold - we will calculate SAF all cells with more than 0.25 snc (previously used 0.1)
 ...
 foreach var (albedo tas snc)
  ...
  if ($var == albedo) then
    cdo setname,albedo -ymonmean -selname,$var -setvrange,0.0,1.0 -setmissval,-999.0 -seldate,$dates -sellonlatbox,${regstr} $infile tmp.albedo.nc || exit 1
  else if ($var == snc) then
    set gridstr = ""
  # Added check for using selgrid
  set flag = `cdo info $infile | grep "1 : generic"`
  if ($#flag != 0 ) set gridstr = "-selgrid,2"
    cdo ymonmean -selname,$var -setvrange,0.0,100.0 -setmissval,-999.0 -seldate,$dates -sellonlatbox,${regstr} $gridstr $infile tmp.snc.nc || exit 1
 ...
 while ($mon <= 6)
  ...
  cdo selmon,$mon tmp.albedo.nc tmp.alb.nc || exit 1
  cdo setname,alpha_land -setmon,$mon -min tmp.alb.nc tmp.alpha_land.nc tmp.alpha_land.0$mon.nc || exit 1
  cdo ifthen tmp.alpha_land.nc tmp.alpha_land.0$mon.nc tmp.alpha_land.mk.nc || exit 1
  \mv tmp.alpha_land.mk.nc tmp.alpha_land.0$mon.nc || exit 1

  cdo selmon,$mon tmp.albedo.nc tmp2.nc || exit 1
  cdo selmon,$mon tmp.snc.nc tmp3.nc || exit 1
...
#
 ...
 set save_vars = ( max_albedo alpha_land.wgt delta_albedo delta_snc delta_tas mean_alpha_snow.wgt delta_alpha_snow albedo.wgt albedo snc tas qh_aland.wgt qh_asnow.wgt mean_snow) || exit 1
 ...
 foreach k (k1 k2 k1k2 k3_res k3 k4 k5 alpha_land.wgt delta_albedo delta_snc delta_tas mean_alpha_snow.wgt delta_alpha_snow albedo.wgt albedo snc tas qh_aland.wgt qh_asnow.wgt mean_snow) || exit 1
